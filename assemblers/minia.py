# -*- coding: utf-8 -*-
"""
Created on Sun May  5 19:00:08 2019
...
class Minia:
    ...
    def __init__(self, technology, exp, out, sample,read_len,file_format,k,t):
        ...
        try:
            ...
            if file_format=="fa" or file_format=="fasta" or file_format=="fna":
                ...
                self.tipo="fastq"
            if not(os.path.exists(out+"assemblies/minia/")):
                os.system("mkdir "+out+"assemblies/minia/")  
            os.system("mkdir "+out+"assemblies/minia/"+sample)
        except IOError:
    ...
    def run(self):
        ...
        try:
            manifest=open(self.out+"assemblies/minia/"+self.sample+"/minia.txt","w")
            path= os.getcwd()
            manifest.write(self.out+"reads/"+self.sample+"_1."+self.file_format+"\n"+self.out+"reads/"+self.sample+"_2."+self.file_format)
            manifest.close()
            command = "cd "+self.out+"assemblies/minia/"+self.sample+";"+path+"/assemblers/minia/build/bin/minia -in "+self.out+"assemblies/minia/"+self.sample+"/minia.txt -kmer-size "+str(self.k)+" -nb-cores "+str(self.t)+" | tee -a " +self.out + self.exp+ ".log"
            os.system(command)           
        except IOError:
            logging.error(IOError)
            exit()
